...
class SegmentTree:
    ...
    def update(self, qleft, qright, qval, left, right, pos):
        ...
        if qleft <= self.xs[left] and self.xs[right+1] <= qright:
            ...
            mid = (left+right) // 2
            self.update(qleft, qright, qval, left, mid, pos * 2)
            self.update(qleft, qright, qval, mid + 1, right, pos * 2 + 1)
        ...
        if self.count[pos] > 0:
            ...
            if left == right:
                ...
                self.covered[pos] = self.covered[pos * 2] + self.covered[pos * 2 + 1]
...
class Solution:
    def separateSquares(self, squares: List[List[int]]) -> float:
        ...
        for y, delta, xl, xr in events:
            ...
            seg_tree.update(xl, xr, delta, 0, seg_tree.n - 2, 1)
        ...
        if width == 0:
            return float(height)
        return height + (target - area) / width
